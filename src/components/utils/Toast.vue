...
onUpdated(() => {
  const all = props.errors.filter((obj: any) => {
    return obj.show != true;
  });
  all.forEach(function (error: any) {
    var errorToast: any = document.getElementById(error.id);
    var toast = new Toast(errorToast);
    toast.show();
    error.show = true;
    errorToast.addEventListener("hidden.bs.toast", function () {
      const indexOfObject = props.errors.findIndex((item : any) => {
...
<script lang="ts">
const TOASTS_MAX = 5;
export function push(array: Array<any>, data) {
